#include "Solver.hpp"
#include "Macro.hpp"
Solver::Solver(string methods) { flag = methods; };
...
void Solver::read_parameter_files() {
  ...
  if (flag == "ACO") {
    ...
    // aco = new SACO (num_of_ants,alpha1, beta1, 1, ro, taumax,0, alpha_SA,
    // beta_SA,  t_current,  t_end);
  } else {
    ifstream fin("./Parameter/GA.txt");
    int number;
    double ht;
    while ((fin.getline(line, CHAR_LEN - 1))) {
      if (!(keywords = strtok(line, Delimiters))) {
    ...
    int_parameter.push_back(number);
    double_parameter.push_back(ht);
  }
}
...
void Solver::end_run(int r) {
  ...
  cout << "End of run " << r << " with best solution quality "
       << problem->current_best << " total evaluations: " << problem->evals
       << endl;
  cout << " " << endl;
}
...
void Solver::init_algorithm(int run) {
  if (flag == "ACO") {
    aco = new SACO(int_parameter[0], double_parameter[0], double_parameter[1], int_parameter[1],
                  double_parameter[2], double_parameter[3], 0,
                  double_parameter[4], double_parameter[5], double_parameter[6],
                  double_parameter[7]);
    aco->init(run);
  } else {
    ga = new GA(int_parameter[0], double_parameter[0]);
    ga->init();
  }
}
...
void Solver::get_mean(int r, double value) { perf_of_trials[r] = value; }
