#include "Solver.hpp"
#include "Macro.hpp"
Solver::Solver(string methods, char*list, Problem &problem, FileOperator  &operatorrr){
    flag  = methods;
    midlist = list;
    problemt = &problem;
...
bool Solver::termination_condition(double rate) {
  ...
  bool flag;
  if(problemt->evals >= rate * problemt->TERMINATION)
    flag = true;
  else
    flag = false;
...
void Solver::end_run(int r){
  operatorr->get_mean(r-1,problemt->current_best); //from stats.h
  cout << "End of run " << r << " with best solution quality " << problemt->current_best << " total evaluations: " << problemt->evals << endl;
  cout << " " << endl;
}
